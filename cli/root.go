// Package cli assembles the chembl command tree from the chembl
...
// NewApp assembles the kit application from the chembl domain. The
...
//
// To add a command, declare it in chembl/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := chembl.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(chembl.Domain{}).Register(app)
