...
describes what users get. Pick a format with `-o`, or let chembl choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
chembl <command> -o table   # aligned columns for reading
chembl <command> -o jsonl   # one JSON object per line, for piping
chembl <command> -o json    # a single JSON array
chembl <command> -o csv     # spreadsheet friendly
chembl <command> -o tsv     # tab-separated
chembl <command> -o url     # just the URL column
chembl <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
chembl <command> --fields id,title,url
```
...
```bash
chembl <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
chembl <command>            # a table, because this is a terminal
chembl <command> | wc -l    # JSONL, because this is a pipe
```
